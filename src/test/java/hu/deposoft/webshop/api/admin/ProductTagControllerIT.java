...
class ProductTagControllerIT {
    ...
    void unauthListReturns401() throws Exception {
        mvc.perform(get("/api/admin/products/tags")).andExpect(status().isUnauthorized());
    }

    @Test
    void createRenameThenDelete() throws Exception {
        // POST → create tag with slug "kezi"
        MvcResult result = mvc.perform(post("/api/admin/products/tags").with(admin()).with(csrf())
        ...
        mvc.perform(put("/api/admin/products/tags/" + id).with(admin()).with(csrf())
                        .contentType("application/json")
        ...
        // DELETE → 204
        mvc.perform(delete("/api/admin/products/tags/" + id).with(admin()).with(csrf()))
                .andExpect(status().isNoContent());
    }

    @Test
    void duplicateSlugReturns400() throws Exception {
        mvc.perform(post("/api/admin/products/tags").with(admin()).with(csrf())
                        .contentType("application/json")
                        .content("{\"name\":\"Kézi\",\"slug\":\"kezi\"}"))
                .andExpect(status().isOk());
    ...
    void renameUnknownReturns404() throws Exception {
        mvc.perform(put("/api/admin/products/tags/999999").with(admin()).with(csrf())
                        .contentType("application/json")
                        .content("{\"name\":\"x\"}"))
    ...
    void deleteUnknownReturns404() throws Exception {
        mvc.perform(delete("/api/admin/products/tags/999999").with(admin()).with(csrf()))
                .andExpect(status().isNotFound());
    }

    @Test
    void listReturnsWithTotalCount() throws Exception {
        mvc.perform(get("/api/admin/products/tags").with(admin()))
                .andExpect(status().isOk())
    ...
    void getByIdReturnsTag() throws Exception {
        MvcResult created = mvc.perform(post("/api/admin/products/tags").with(admin()).with(csrf())
                        .contentType("application/json")
        ...
        mvc.perform(get("/api/admin/products/tags/" + id).with(admin()))
                .andExpect(status().isOk())
    ...
    void getByIdUnknownReturns404() throws Exception {
        mvc.perform(get("/api/admin/products/tags/999999").with(admin()))
                .andExpect(status().isNotFound());
    }
}
